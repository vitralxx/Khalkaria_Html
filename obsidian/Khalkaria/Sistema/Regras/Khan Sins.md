---
tipo: regra
status: canon-notion
spoiler: publico
era: atemporal
aliases: [Khan Sins, Jogo de apostas, Buy-In, Mesa]
fonte_notion: 2b76e3a401d980c0ab7bc27a342f4ee1
ultima_sync: 2026-09-05
tags: [khalkaria/sistema, khalkaria/sistema/regras]
---

# Khan Sins

**Resumo.** Jogo de apostas popular entre os locais e os povos antigos de [[Kharavel]]. O jogador entra com um **Buy-In** limitado pelo teto da mesa e rola **1d20 + proficiência na perícia da mesa** contra uma CD fixa ou contra o d20 da mesa. Sucesso multiplica o Buy-In; derrota leva tudo. Qualquer lado pode **roubar** (Crime × Percepção) e quem rouba com sucesso ganha na hora.

## Como se joga

> Pelos ermos de Khalkaria, Khan Sins é um jogo de apostas muito popular entre os locais e povos antigos de Kharavel.
> O jogo se chama "Khan Sins"
> O jogador primeiro, escolhe com quantos Sins entrar, limitado pelo teto de Sins da mesa.
> O jogador rola 1d20 + Proficiência na perícia da mesa.
> Cada mesa possui uma perícia base, quanto mais rara/exótica a perícia, maior o Multiplicador Base da mesa.
> Se a mesa for fixa, o jogador compara o resultado com a CD da mesa.
> Se a mesa for dinâmica, o jogador compara o resultado com a rolagem d20 da mesa.
> Se o jogador suceder, ele multiplica seu Buy-In pelo Multiplicador Base da mesa.
> Se o jogador perder, ele perde todo o buy in.
> O jogador, assim como a mesa, pode escolher roubar, realizando um teste de crime comparado a um teste de percepção da outra parte. Se suceder, o ladrão instantaneamente ganha. Caso a mesa roube, o player utiliza percepção passiva como CD.

## As três mesas

| Tier | CD Vitória | Perícias + Mult. | Mult. Buy-In | Crime | Percepção | Teto Buy-In |
|---|---|---|---|---|---|---|
| Rua | 12 ou 1d20+2 | Reflexos (+0.5), Percepção (+0), Movimento (+0), Furtividade (+1) | x1.5 | +2 | +5 | 25 |
| Taverna | 15 ou 1d20+5 | Sobrevivência (+0.5), Investigação (+0.5), Conhecimento (+0.5), Fortitude (+1), Iniciativa(+1) | x2 | +5 | +10 | 50 |
| Khan | 20 ou 1d20+10 | Vontade (+1), Místico (+1), Crime (+1), Religião (+2), Ofício (+2), Medicina (+2) | x3 | +10 | +15 | Sem Teto |

## Variações locais

> Note, cada mesa através de Kharavel pode modificar/adicionar certas exceções ou adições nas regras de Khan Sins. Nunca se sabe o que tem por aí.

## Relações
- **Faz parte de** → [[Sistema]]
- **Aposta** → Sins ([[Dinheiro e Comerciantes]])
- **Usa** → [[Atributos e Perícias]] (a perícia da mesa, Crime, Percepção)
- **Jogado em** → [[Kharavel]], sobretudo em tavernas — ver [[Taverna de Khaskavel]]
- **Nome ecoa** → [[Malkhor]], o Pecado, e a moeda Sins

## Pontas soltas
- **O "+Mult." das perícias** (+0.5, +1, +2) não é explicado: soma ao Multiplicador Base da mesa, multiplica o Buy-In, ou entra na rolagem? [[Perguntas Abertas]] E52.
- **"Percepção passiva"** serve de CD quando a mesa rouba, mas o sistema nunca define percepção passiva. [[Perguntas Abertas]] E52.
- **De onde vem o nome.** Nada conta a origem de "Khan" nem se "Sins" ecoa deliberadamente o Pecado de [[Malkhor]].

## Procedência
- Notion `2b76e3a401d980c0ab7bc27a342f4ee1` — Khan Sins.
- `docs/memoria/notion_raw/sistema_root.md`; digest `A_sistema.md` §1.19.
- Revisão de redação: 2026-09-07.
